...
import com.toonystank.requisiteteams.gui.system.components.util.GuiFiller;
...
public abstract class GUISystem implements InventoryHolder {

    // The plugin instance for registering the event and for the close delay.
    private static final Plugin plugin = JavaPlugin.getProvidingPlugin(GUISystem.class);
    ...
    private final ConcurrentMap<Integer,GuiItem> tempItemMapForAIR = new ConcurrentHashMap<>();
    ...
    // Whether you should run the actions from the close and open methods.
    private boolean runCloseAction = true;
    private final boolean runOpenAction = true;
     ...
     * @since 3.0.0.
     */
    public GUISystem(final int rows, @NotNull final Component title, @NotNull final Set<InteractionModifier> interactionModifiers) {
        int finalRows = rows;
     ...
     * @since 3.0.0
     */
    public GUISystem(@NotNull final GuiType guiType, @NotNull final Component title, @NotNull final Set<InteractionModifier> interactionModifiers) {
     ...
     */
    @Deprecated
    public GUISystem(final int rows, @NotNull final Component title) {
        int finalRows = rows;
        if (!(rows >= 1 && rows <= 6)) finalRows = 1;
        this.rows = finalRows;
     ...
     * @param guiType The {@link GuiType} to use.
     * @param title   The GUI title.
     * @deprecated In favor of {@link GUISystem#GUISystem(GuiType, Component, Set)}.
     */
    @Deprecated
    public GUISystem(@NotNull final GuiType guiType, @NotNull final Component title) {
        this.guiType = guiType;
        this.interactionModifiers = EnumSet.noneOf(InteractionModifier.class);
        this.title = title;
     ...
     * Updates the specified item in the GUI at runtime, without creating a new {@link GuiItem}.
     *
     * @param slots      The slot of the item to update.
     * @param itemStack The {@link ItemStack} to replace in the original one in the {@link GuiItem}.
     */
    public void updateItem(final List<Integer> slots, @NotNull final ItemStack itemStack) {
    ...
    public void updateItem(final List<Integer> slots, @NotNull final GuiItem guiItem) {
        for (Integer slot : slots) {
            updateItem(slot, guiItem);
        }
    }
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem disableItemPlace() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem disableItemTake() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem disableItemSwap() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem disableItemDrop() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem disableOtherActions() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem disableAllInteractions() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem enableItemPlace() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem enableItemTake() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem enableItemSwap() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem enableItemDrop() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem enableOtherActions() {
    ...
    @NotNull
    @Contract(" -> this")
    public GUISystem enableAllInteractions() {
    ...
    void populateGui() {
        for (final Map.Entry<Integer, GuiItem> entry : guiItems.entrySet()) {
            if (entry.getValue().isAir()) {
                tempItemMapForAIR.put(entry.getKey(), entry.getValue());
            }
            inventory.setItem(entry.getKey(), entry.getValue().getItemStack());
        }
    }
    public void replaceAirItem() {
        for (Map.Entry<Integer, GuiItem> entry : tempItemMapForAIR.entrySet()) {
