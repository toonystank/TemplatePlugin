...
public abstract class BaseItemBuilder<B extends BaseItemBuilder<B>> {
    ...
    public B lore(@NotNull final String... lore) {
        return lore(Arrays.stream(lore).map(MessageUtils::format).toList());
    }
    public B lore(@NotNull final List<String> lore,boolean s) {
        return lore(lore.stream().map(MessageUtils::format).toList());
    }
    ...
    public B lore(@NotNull final Consumer<List<@Nullable Component>> lore) {
        ...
        // Get existing lore using PaperMC's native method
        List<Component> components = meta.lore() != null ? new ArrayList<>(meta.lore()) : new ArrayList<>();
    ...
    public B model(final int modelData) {
        MessageUtils.debug("Setting custom model data: " + modelData);
        if (this.meta == null) {
            setMeta();
            if (this.meta == null) return (B) this;
        }
    ...
    @NotNull
    @Contract("_ -> new")
    public GuiItem asGuiItem(@NotNull final GuiClickAction action) {
