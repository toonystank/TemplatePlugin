...
/**
 * Requirement that checks if one string contains another.
 */
public class StringContains extends Requirements {
    private String input;
    private String output;

    public StringContains(String path, ItemSection section) {
        super(RequirementTypes.STRING_CONTAINS, path, section);
    }
    ...
    public boolean parse(@NotNull RequisitePlayer player, @Nullable List<String> args, @NotNull BaseGUI manager) {
        ...
        String formattedOutput = manager.getGuiData().formatText(player, input, args, true,false);
        return formattedInput != null && formattedOutput != null && formattedInput.contains(formattedOutput);
    }
}